...
export const SCENES: SceneDefinition[] = [
  {
    ...
    draw: (ctx, audioData, settings, webcamFeed) => {
      ...
      if (webcamFeed && settings.showWebcam && webcamFeed.readyState >= webcamFeed.HAVE_METADATA && webcamFeed.videoWidth > 0 && webcamFeed.videoHeight > 0) {
        ...
        // Use SBNF accent for difference blending
        const sbnfAccentColor = getComputedStyle(ctx.canvas).getPropertyValue('--accent').trim() || 'hsl(36, 98%, 63%)';

        const differenceAlpha = Math.min(1, (0.95 + audioData.rms * 0.1 + (audioData.beat ? 0.05 : 0)) * settings.brightCap);
        ctx.fillStyle = `hsla(${sbnfAccentColor.startsWith('hsl') ? sbnfAccentColor.match(/hsl\(([^,]+)/)?.[1] : '36'}, 90%, 70%, ${differenceAlpha})`;
        ctx.fillRect(0, 0, width, height);
  ...
  },
  {
    id: 'echoing_shapes',
    name: 'Echoing Shapes',
    thumbnailUrl: 'https://placehold.co/120x80/FF441A/FFECDA.png?text=Echoes', // SBNF Colors
    ...
    draw: (ctx, audioData, settings) => {
      ...
      if (audioData.beat || audioData.rms > 0.02) {
        ...
        for (let i = 0; i < numShapes; i++) {
          ...
          // Use SBNF accent colors for shapes
          const hueOptions = [13, 36, 267]; // Orange-Red, Orange-Yellow, Light Lavender
  ...
  },
   {
    ...
    thumbnailUrl: 'https://placehold.co/120x80/FDB143/000000.png?text=Rings', // SBNF Colors
  ...
  },
  {
    ...
    thumbnailUrl: 'https://placehold.co/120x80/E1CCFF/5A36BB.png?text=Grid', // SBNF Colors
  ...
  },
  {
    id: 'spectrum_bars',
    name: 'Spectrum Bars',
    thumbnailUrl: 'https://placehold.co/120x80/5A36BB/FDB143.png?text=Bars', // SBNF Colors
  ...
  },
   {
    id: 'radial_burst',
    name: 'Radial Burst',
    thumbnailUrl: 'https://placehold.co/120x80/FF441A/000000.png?text=Burst', // SBNF Colors
    ...
    draw: (ctx, audioData, settings) => {
      ...
      // SBNF Hues for particles
      const sbnfHues = [13, 36, 267, 258]; // Orange-Red, Orange-Yellow, Lavender, Deep Purple
      ...
      if (audioData.beat) {
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          const hue = (sbnfHues[i % sbnfHues.length] + audioData.bassEnergy * 50 + (Math.random() * 40 - 20)) % 360; 
  ...
  },
  {
    ...
    thumbnailUrl: 'https://placehold.co/120x80/5A36BB/FF441A.png?text=Tunnel', // SBNF Colors
  ...
  },
  {
    id: 'strobe_light',
    name: 'Strobe Light',
    thumbnailUrl: 'https://placehold.co/120x80/FFECDA/000000.png?text=Strobe', // SBNF Colors
    dataAiHint: 'strobe light flash',
    draw: (ctx, audioData, settings) => {
      const { width, height } = ctx.canvas;
      if (audioData.beat && settings.brightCap > 0.01) {
        const hueOptions = [13, 36, 267]; // Orange-Red, Orange-Yellow, Lavender
        const hue = hueOptions[Math.floor(Math.random() * hueOptions.length)];
  ...
  },
  {
    ...
    draw: (ctx, audioData, settings) => {
      ...
      const centerX = width / 2;
      const centerY = height / 2;
      const MAX_AMBIENT_PARTICLES = 700;
      const MAX_BURST_PARTICLES = 1500;
      const sbnfHues = [13, 36, 267, 258]; // SBNF Palette

      const ambientParticleCount = Math.min(MAX_AMBIENT_PARTICLES, 80 + Math.floor(audioData.rms * 200 + audioData.midEnergy * 120 + audioData.trebleEnergy * 80));
      ...
      if (audioData.beat) {
        const burstParticleCount = Math.min(MAX_BURST_PARTICLES, 250 + Math.floor(audioData.bassEnergy * 600 + audioData.rms * 400));
