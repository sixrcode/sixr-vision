...
export const DEFAULT_SETTINGS: Settings = {
  ...
  brightCap: 1.0,
  logoOpacity: 0.25,
  showWebcam: false, // Webcam OFF by default, controlled by dedicated toggle
  mirrorWebcam: false,
...
export const SCENES: SceneDefinition[] = [
  {
    ...
    draw: (ctx, audioData, settings) => {
      ...
      ctx.fillStyle = `hsla(var(--background-hsl), ${settings.sceneTransitionActive && settings.sceneTransitionDuration > 0 ? 1 : 0.9})`;
  ...
  },
  {
    ...
    draw: (ctx, audioData, settings) => {
      const { width, height } = ctx.canvas;
      ctx.fillStyle = `hsla(var(--background-hsl), ${settings.sceneTransitionActive && settings.sceneTransitionDuration > 0 ? 0.18 : 0.12})`; // Slightly faster fade
      ctx.fillRect(0,0,width,height);
      ...
      const numStaticParticles = 40 + Math.floor(audioData.rms * 80); 
      for (let i = 0; i < numStaticParticles; i++) {
        ...
        const maxRadius = Math.min(width, height) * (0.1 + audioData.midEnergy * 0.25);  
        const currentRadius = maxRadius * (0.25 + energy * 0.75);
        const x = centerX + Math.cos(angle) * currentRadius;
        const y = centerY + Math.sin(angle) * currentRadius;
        const particleSize = (1 + energy * 3) * settings.brightCap; 
        const hue = 170 + energy * 90 + (audioData.beat ? 25 : 0); 
        ctx.fillStyle = `hsla(${hue % 360}, ${90 + energy*10}%, ${60 + energy*20}%, ${0.35 + energy * 0.6})`; 
        ctx.beginPath();
      ...
      if (audioData.beat) {
        const particleCount = 100 + Math.floor(audioData.rms * 250 + audioData.bassEnergy * 200); 
        for (let i = 0; i < particleCount; i++) {
          ...
          const size = (2.5 + Math.random() * 8 * (audioData.rms + audioData.bassEnergy * 0.7)) * settings.brightCap; 
          const hue = ((audioData.bassEnergy * 70) + (Math.random() * 30 - 15) + 360) % 360; 
    ...
    draw: (ctx, audioData, settings, webcamFeed) => {
      ...
      if (webcamFeed && settings.showWebcam && webcamFeed.readyState >= webcamFeed.HAVE_METADATA && webcamFeed.videoWidth > 0 && webcamFeed.videoHeight > 0) {
        ...
        if (camAspect > canvasAspect) { 
            dHeight = height;
            dWidth = height * camAspect;
            dx = (width - dWidth) / 2;
            dy = 0;
        } else { 
            dWidth = width;
            dHeight = width / camAspect;
            dy = (height - dHeight) / 2;
            dx = 0;
        }
        ...
        ctx.drawImage(webcamFeed, 0, 0, camWidth, camHeight, dx, dy, dWidth, dHeight);
  ...
  },
  {
    ...
    thumbnailUrl: 'https://placehold.co/120x80/1f2937/f472b6.png?text=Explosion',
    dataAiHint: 'particle explosion fireworks',
    draw: (ctx, audioData, settings) => {
      const { width, height } = ctx.canvas;
      ctx.fillStyle = `hsla(var(--background-hsl), ${settings.sceneTransitionActive && settings.sceneTransitionDuration > 0 ? 0.2 : 0.15})`; 
      ctx.fillRect(0, 0, width, height);
      ...
      const ambientParticleCount = Math.min(MAX_AMBIENT_PARTICLES, 20 + Math.floor(audioData.rms * 50)); 
      ...
      if (audioData.beat) {
        const burstParticleCount = Math.min(MAX_BURST_PARTICLES, 70 + Math.floor(audioData.bassEnergy * 80 + audioData.rms * 60)); 
        for (let i = 0; i < burstParticleCount; i++) {
          const angle = Math.random() * Math.PI * 2;
          const radius = Math.random() * Math.min(width, height) * 0.55 * (0.35 + audioData.bassEnergy * 0.3 + audioData.rms * 0.25); 
  ...
  },
  {
    ...
    draw: (ctx, audioData, settings) => {
      const { width, height } = ctx.canvas;
      ctx.fillStyle = `hsla(var(--background-hsl), 0.4)`; 
      ctx.fillRect(0, 0, width, height);

      const gridSize = 7 + Math.floor(audioData.rms * 10); 
      ...
      for (let i = 0; i < gridSize; i++) {
        for (let j = 0; j < gridSize; j++) {
          ...
          const beatFactor = audioData.beat ? 1.75 : 1.0; 
          const maxRadius = maxRadiusBase * beatFactor;

          const centerX = i * cellWidth + cellWidth / 2;
          const centerY = j * cellHeight + cellHeight / 2;

          const radius = maxRadius * energy * settings.brightCap * (0.3 + audioData.rms * 0.6 + audioData.bassEnergy * 0.3); 
    ...
    draw: (ctx, audioData, settings) => {
      ...
      for (let i = 0; i < energies.length; i++) {
        ...
        for (let j = 0; j < numSteps; j++) {
            ...
            const time = performance.now() / (1000 / (speedFactor * 0.7 + 0.3)); 
            const ringProgress = (time + j * (0.5 / numSteps) * (i + 1.2)) % 1; 

            const radius = ringProgress * maxRingRadius * (0.4 + energy * 0.6); 
            if (radius < 1) continue;

            const alpha = (1 - ringProgress) * energy * settings.brightCap * 2.0; 
            if (alpha <= 0.01) continue;

            const thickness = (2.0 + energy * 15 + (audioData.beat ? 4.0 : 0)) * settings.brightCap; 
  ...
  },
  {
    ...
    draw: (ctx, audioData, settings) => {
      ...
      if (audioData.beat && audioData.rms > 0.035) { 
        ...
        const saturation = 30 + audioData.trebleEnergy * 70; 
  ...
  },
  {
    ...
    draw: (ctx, audioData, settings) => {
      ...
      ctx.fillStyle = `hsla(var(--background-hsl), ${settings.sceneTransitionActive && settings.sceneTransitionDuration > 0 ? 0.07 : 0.05})`; 
      ctx.fillRect(0, 0, width, height);

      if (audioData.beat || audioData.rms > 0.07) { 
        const numShapes = 3 + Math.floor(audioData.rms * 12 + audioData.bassEnergy * 8); 
        for (let i = 0; i < numShapes; i++) {
          const sizeBase = (10 + audioData.bassEnergy * 130 + Math.random() * 50); 
          ...
          const alpha = (0.25 + audioData.trebleEnergy * 0.8 + audioData.rms * 0.4) * settings.brightCap; 

          ctx.fillStyle = `hsla(${hue}, 95%, ${55 + Math.random()*15}%, ${Math.min(1, alpha)})`;
          ctx.save();
          ctx.translate(x,y);
          ctx.rotate( (performance.now() / 800 + i) * (audioData.trebleEnergy * 0.6 + 0.1) ); 
  ...
  },
  {
    ...
    draw: (ctx, audioData, settings) => {
      ...
      ctx.fillStyle = `hsla(var(--background-hsl), ${settings.sceneTransitionActive && settings.sceneTransitionDuration > 0 ? 0.5 : 0.4})`; 
      ctx.fillRect(0, 0, width, height);

      const numLayers = 10 + Math.floor(audioData.rms * 10); 
      const maxDepth = Math.min(width, height) * 2.0; 

      for (let i = 0; i < numLayers; i++) {
        const timeFactor = performance.now() / (1800 - audioData.bpm * 7.0); 
        ...
        ctx.lineWidth = Math.max(0.8, (1 - depthProgress) * (8 + (audioData.beat ? 5.5 : 0)) * settings.brightCap); 

        ctx.save();
        ctx.translate(centerX, centerY);
        const rotationSpeed = (audioData.trebleEnergy - 0.3) * 0.3; 
